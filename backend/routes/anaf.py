                    ...
                    async with aiohttp.ClientSession() as http_session:
                        async with http_session.post(
                            ...
                            if response.status == 200:
                                ...
                                state.add_anaf_log(f"⚠️ Batch {current_batch}: Rate limit (429), aștept 15s...")
                                await asyncio.sleep(15)
                            else:
                                state.add_anaf_log(f"✗ Batch {current_batch}: Eroare API status {response.status}")
                                if attempt < 2:
                                    await asyncio.sleep(5 * (attempt + 1))
                except Exception as e:
                    error_msg = str(e)[:50]
                    if attempt < 2:
                        state.add_anaf_log(f"⚠️ Batch {current_batch}: {error_msg} - retry...")
                        await asyncio.sleep(5 * (attempt + 1))
                    else:
                        state.add_anaf_log(f"✗ Batch {current_batch}: Eroare după 3 încercări - {error_msg}")
